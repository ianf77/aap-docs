...
The *Automation Templates* page displays cards with the workflow and job templates that are currently available. 
//he default view is collapsed (Compact), showing the template name, template type, and the statuses of the jobs that have run by using that template. 
...
From each template card you can launch image:rightrocket.png[Launch,15,15], edit image:leftpencil.png[Edit,15,15], and duplicate image:copy.png[Copy,15,15] a template.
